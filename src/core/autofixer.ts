...
export class Autofixer {
  ...
  private buildWorkflowYml(): string {
    return [
      ...
      `      - name: Install jira-autofix`,
      `        run: bun add jira-autofix`,
      `      - name: Fetch issues`,
      `        id: fetch`,
      `        run: |`,
      `          bun run jira-autofix-fetch && echo "has_issues=true" >> \$GITHUB_OUTPUT || echo "has_issues=false" >> \$GITHUB_OUTPUT`,
      `        env:`,
      ...
      `        run: bun run jira-autofix-open-prs`,
      `        env:`,
      `          JIRA_BASE_URL: \${{ secrets.JIRA_BASE_URL }}`,
      `          JIRA_EMAIL: \${{ secrets.JIRA_EMAIL }}`,
      `          JIRA_API_TOKEN: \${{ secrets.JIRA_API_TOKEN }}`,
